...
def main():
    ...
    parser.add_argument('--artifactory_user',
                        default=os.getenv('artifactory_user'))
    parser.add_argument('--artifactory_key',
                        default=os.getenv('artifactory_key'))
    parser.add_argument('--artifactory_pass',
                        default=os.getenv('artifactory_pass'))
    parser.add_argument('--artifactory_url', default=os.getenv('artifactory_url',
                        "https://artifactory.devops.telekom.de/artifactory"))
    parser.add_argument('--artifactory_repo',
                        default=os.getenv('artifactory_repo', "schiff-generic"))
    parser.add_argument(
        '--upload_mode', default=os.getenv('upload_mode', "govc"))
    parser.add_argument('--bw_user', default=os.getenv('bw_user'))
    parser.add_argument('--bw_pass', default=os.getenv('bw_pass'))
    parser.add_argument('--bw_url', default=os.getenv('bw_url',
                        "https://bitwarden.das-schiff.telekom.de"))
    parser.add_argument('--enable_bw', default=False)
    parser.add_argument('--vcenter_config_file',
                        type=argparse.FileType('r'), nargs="?", default=".bin/dummy.yaml")
    exit = False
    error_occured = True
    ...
    if args.artifactory_user is None and args.artifactory_key is not None:
        ...
        if args.artifactory_key is not None:
            artifactory_auth = (str(args.artifactory_user),
                                str(args.artifactory_key))
            ...
            logging.error(
                "Set either artifactory API Key or User and Password/API-Key")
            exit = True
    ...
    if args.enable_bw:
        if args.bw_pass is None and args.bw_user is None:
            logging.error("Set Bitwarden User and Pass")
            exit = True
    ...
    if args.enable_bw:
        process = subprocess.run(["bw", "config", "server", str(
            args.bw_url)], check=True, stdout=subprocess.PIPE, universal_newlines=True)
        output = process.stdout
        logging.error(output)
        try:
            process = subprocess.run(["bw", "login", args.bw_user, args.bw_pass, "--raw"],
                                     check=True, stdout=subprocess.PIPE, universal_newlines=True)
            output = process.stdout
            os.environ['BW_SESSION'] = output
            logging.error("Set BW_SESSION")
        except subprocess.CalledProcessError as err:
            if "You are already logged in as" in err.output:
                logging.info("Already logged in")
                pass
        process = subprocess.run(["bw", "get", "username", "vcenter1.sce-dcn.net"],
                                 check=True, stdout=subprocess.PIPE, universal_newlines=True)
        output = process.stdout
        print(output)
        process = subprocess.run(["bw", "get", "password", "vcenter1.sce-dcn.net"],
                                 check=True, stdout=subprocess.PIPE, universal_newlines=True)
        output = process.stdout
        print(output)
        process = subprocess.run(
            ["bw", "logout"], check=True, stdout=subprocess.PIPE, universal_newlines=True)
        output = process.stdout
        print(output)
    ...
    if args.upload_mode == "govc":
        process = subprocess.run(["govc", "version"], check=True, stdout=subprocess.PIPE,
                                stderr=subprocess.STDOUT, universal_newlines=True)
        logging.info(process.stdout)
    for p in artifacts:

        fileurl = args.artifactory_url + "/" + \
            p["repo"] + "/" + p["path"] + "/" + p["name"]
        path = ArtifactoryPath(fileurl, session=ses)
        stat = path.stat()
        
        if os.path.isfile(p["name"]):
            checksum = md5(p["name"])
            logging.info(f'file: {p["name"]}')
            logging.info(f'current: {checksum} remote: {stat.md5}')
            if checksum != stat.md5:
                os.remove(p["name"])
        if not os.path.isfile(p["name"]):
            logging.info(f'DOWNLOADING file: {p["name"]}')
            path.writeto(out=p["name"])
            ...
            checksum = md5(p["name"])
            if checksum != stat.md5:
                logging.error("Filedownload failed, checksums arent matching")
                logging.error(f'current: {checksum} remote: {stat.md5}')
        ...
        for vcenter in vcenter_data["vcenters"]:
            if args.upload_mode == "python":
                try:
                    ...
                    logging.error("Upload for vCenter " +
                                  vcenter["host"]+" failed")
                    logging.error("Reason:")
                    logging.error(e)
                    error_occured = True
            elif args.upload_mode == "ovftool":
                subprocess_args = ["ovftool", f'--diskMode=thin', '--allowExtraConfig', f'--extraConfig:disk.enableUUID=1', 
                                   f'--noNvramFile', f'--importAsTemplate', f'--noDestinationSSLVerify', p["name"],
                                    f'vi://{vcenter["user"]}:{vcenter["password"]}@{vcenter["host"]}{vcenter["resource_pool"]}']
                if os.environ.get('https_proxy') is not None and "10.208.220.80" not in os.environ.get('https_proxy'):
                    subprocess_args.insert(2, f"--proxy={os.environ.get('https_proxy')}")
                if "datastore_cluster" in vcenter:
                    subprocess_args.insert(2,f'-ds={vcenter["datastore_cluster"]}')
                else:
                    subprocess_args.insert(2,f'-ds={vcenter["datastore"]}')
                if "/" in vcenter["network"]:
                    subprocess_args.insert(2,f'--network={vcenter["network"].rsplit("/",1)[1]}')
                else:
                    subprocess_args.insert(2,f'--network={vcenter["network"]}')
                if vcenter["datacenter"] in vcenter["folder"]:
                    subprocess_args.insert(2,f'--network={vcenter["folder"].replace(vcenter["datacenter"],"").replace("/vm","").replace("//","/")}')
                else:
                    subprocess_args.insert(2,f'--network={vcenter["folder"]}')
                try:
                    logging.info(
                        "vCenter "+vcenter["host"] + " " + vcenter["name"])
                    process = subprocess.run(subprocess_args, check=True,
                ...
                with open("vcenter.pwd", 'w') as outfile:
                    outfile.write(vcenter["password"].replace("$","$$"))
                os.environ['GOVC_PASSWORD'] = "./vcenter.pwd"
                ...
                logging.debug(["govc", "import.spec", p["name"]])
                process = subprocess.run(["govc", "import.spec", p["name"]], check=True,
                                         stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                output = process.stdout
                importoptions = json.loads(output)
                importoptions["NetworkMapping"][0]["Network"] = vcenter["network"]
                importoptions["DiskProvisioning"] = "thin"
                importfile = p["name"]+vcenter["name"]+".json"
                newly_uploaded = False
                with open(importfile, 'w') as outfile:
                    json.dump(importoptions, outfile)
                    logging.debug(importoptions)
                try:
                    process = subprocess.run(["govc", "import.ova", "-debug", "-options="+importfile, p["name"]],
                    ...
                    if "already exists" in err.output:
                        ...
                        logging.error(
                            "vCenter "+vcenter["host"]+" import failed")
                        logging.debug(
                            ["govc", "import.ova", "-debug", "--options="+importfile, p["name"]])
                        logging.error(err.output)
                        if "file does not exist" in err.output:
                            logging.error(os.listdir())
                        continue
                try:
                    if not newly_uploaded:
                        process = subprocess.run(["govc", "vm.markasvm", "-debug", vcenter["folder"]+"/"+p["name"][:-4]],
                                                check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                        output = process.stdout
                        logging.info("remarked as vm")
                        if output:
                            logging.info(output)
                except subprocess.CalledProcessError as err:
                    logging.debug("govc", "vm.markasvm", "-debug",
                                  vcenter["folder"]+"/"+p["name"][:-4])
                    logging.error(err.output)
                try:
                    process = subprocess.run(["govc", "vm.change", "-debug", "-vm", vcenter["folder"]+"/"+p["name"][:-4],
                                             "-e=disk.enableUUID=1"], check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                    output = process.stdout
                    logging.info("disk.enableUUID=1 set")
                    if output:
                        logging.info(output)
                except subprocess.CalledProcessError as err:
                ...
                try:  # "--version="+str(vcenter["vmx_version"])
                    ...
                    if "Virtual machine compatibility is already up-to-date" in err.output:
                        logging.info(err.output)

                    else:
                        logging.debug(["govc", "vm.upgrade", "-debug", "--version="+str(
                            vcenter["vmx_version"]), "-vm", vcenter["folder"]+"/"+p["name"][:-4]])
                        logging.error(err.output)
                try:
                    process = subprocess.run(["govc", "vm.markastemplate", "-debug", vcenter["folder"]+"/"+p["name"][:-4]],
                                             check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                    output = process.stdout
                    logging.info("marked as template")
                    if output:
                        logging.info(output)
                except subprocess.CalledProcessError as err:
                    logging.debug("govc", "vm.markastemplate",
                                  "-debug", vcenter["folder"]+"/"+p["name"][:-4])
                    logging.error(err.output)
    if error_occured:
        return 1
...
def uploadOVA(vcenter_data, ova_path):
    ...
    rp = find_resource_pool_by_cluster(
        content, vcenter_data["resource_pool"], cluster)
    ...
    cisp = vim.OvfManager.CreateImportSpecParams(
        diskProvisioning="thin", networkMapping=nma)
