    ...
    for p in artifacts:
        ...
        for vcenter in vcenter_data["vcenters"]:
            if args.upload_mode == "python":
                ...
                try:
                    print(["govc","import.ova","-debug","--options="+p["name"]+".json",p["name"]])
                    process = subprocess.run(["govc","import.ova","-debug","--options="+p["name"]+".json",p["name"]],check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                    output = process.stdout
                    print(output)
                except subprocess.CalledProcessError as err:
